...
class CustomDataset(Dataset):
    def __init__(self, csv_file, label_col):
        ...
        self.labels = ['rice', 'coffee', 'maize', 'cotton', 'coffee']
        ...
        self.labels = torch.tensor(pd.factorize(self.data[label_col].values)[0], dtype=torch.int64)
...
metrics_calculator = MetricCalculator([classification.f1(num_classes=22)])
    ...
    for initializer_type, value in INITIALIZERS.items():
        for initializer_params in value:
            response = run_experiment(
                test_loader,
                train_loader,
                f"crop_cl_l={model_params['l']}_wk={model_params['w_k']}",
                model,
