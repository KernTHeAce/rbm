...
class CustomDataset(Dataset):
    def __init__(self, csv_file, categorical_col, label_col):
        ...
        self.labels = torch.tensor(self.data[label_col].values, dtype=torch.int64)
        features_data = self.data.drop(columns=[label_col, categorical_col])  # Удаляем таргет и категориальный столбец
...
lengths = {"s": [5], "m": [10, 7, 3], "l": [15, 10, 10, 7, 3]}
